...
void parsebuffer(void *b)
{
  ...
  s2 = buffer_read_nstring(b,12); // TES4SAVEGAME
  printf("%s\n",s2);
  free(s2);

  n = buffer_read_ushort(b);
  printf("%i\n",n);
  n = buffer_read_ushort(b); // Date + time? See below for save date+time.
  printf("%i\n",n);
  n = buffer_read_int(b);
  printf("%i\n",n);
  n = buffer_read_ushort(b);
  ...
  n = buffer_read_ushort(b);
  printf("Save date Year %i\n",n);
  n = buffer_read_int(b);
  printf("Save date month %i %% why int?\n",n);

  n = buffer_read_ushort(b);
  printf("Save date Day %i\n",n);
  n = buffer_read_ushort(b);
  printf("Save time Hours %i\n",n);
  n = buffer_read_ushort(b);
  printf("Save time Mins %i\n",n);
  n = buffer_read_ushort(b);
  printf("%i\n",n);
  n = buffer_read_ushort(b);
  printf("%i\n",n);
  ...
  // Image data follows.
  // 117-
  // tail --bytes +118 autosave.ess |head --bytes 500k > z1
  // cat z1 | rawtoppm -bgr 256 256 > z1.ppm
  buf = buffer_getbuf_atloc(b);
  printf("Raw 256x204 saveimage saved to xsaveimage.rgb\n");
  buffer2file(buf,256*204*3,"xsaveimage.rgb");
  buffer_skip_bytes(b,256*204*3);
  ...
  continueloc = buffer_getloc(b);
  // Read TOC here.

  buffer_setloc(b,tocloc);

  n = buffer_read_int(b);
  printf("Number of TOC Entries %i\n",n);
  numtocentries = n;
  toctable = (int *)malloc(numtocentries*sizeof(int));
  ...
  printf("%f\n",f);
  n = buffer_read_ushort(b);
  printf("Number of int/float pairs %i\n",n);
  num = n;
  for (i = 0; i < num; i++) {
    n = buffer_read_int(b);
    printf("%i\n",n);
  ...
  n = buffer_read_int(b);
  printf("Number of int/ushort pairs %i\n",n);
  num = n;
  for (i = 0; i < num; i++) {
    n = buffer_read_int(b);
    printf("%i\n",n);
