...
// Continue.
// For maitaining the current worldspace/cell.
char global_worldspace[1024];
char global_cell[1024];
int global_cell_x;
int global_cell_y;
...
void parse_ctda_oblivion(void *b)
{
  int n;
  float f;
...
void parse_ctda(void *b, int size)
{
  int n;
  float f;
  int m;
  ...
  //      0000,0000
  printf("        ^0 AND 1 OR\n");
  printf("       ^Run on Target %% See also below for \"Run on\"\n");
  printf("      ^Use Global\n");
  printf("^^^000 == 001 != 010 > 011 >= 100 < 101 <=\n");
  n = buffer_read_ubyte(b);
  printf("%i (unused)\n",n);
  ...
  if (size >= 24) {
    // XXXX 4*ubyte?
    n = buffer_read_int(b);
    printf("Run on %i %% 0 Subject 1 Target 2 Reference 3 Combat Target 4 Linked Reference\n",n);
  }
  if (size >= 28) {
    n = buffer_read_int(b);
    printf("Sub/Tar/Ref FormID %i\n",n);
  }
}
...
void parse_efit_oblivion(void *b)
{
  int n;
  float f;
  char *s2;
  ...
  n = buffer_read_int(b);
  printf("ActorVal %i (for Skill effect) %% XXXX Verify for what\n",n);
}

void parse_efit(void *b) // F
{
  int n;
  float f;
  char *s2;
  ...
  printf("Range %i %% 0 Self 1 Touch 2 Target\n",n);
  n = buffer_read_int(b);
  printf("ActorVal %i (for Skill effect)\n",n);
}
...
// Object Bound. Check if Min and Max points. May not be.
void parse_obnd(void *b)
{
  int n;
...
void parse_coed(void *b)
{
  ...
  n = buffer_read_int(b);
  printf("Global Variable FormID %i\n",n);
  f = buffer_read_float(b);
  printf("Health%% %f %% Editor displays 100x\n",f);
}

void parse_dest(void *b)
{
  int n;
  ...
  printf("        ^Enabled\n");
#else
  printf("Number of Destructible Object Stages %i %% 0 Disabled\n",n);
#endif
  n = buffer_read_ubyte(b);
  printflags8(n);
  //      0000,0000
  printf("        ^VATS Targetable\n");
  n = buffer_read_ubyte(b);
  printf("%i (unused)\n",n);
  n = buffer_read_ubyte(b);
  printf("%i (unused)\n",n);
...
void parse_dstd(void *b)
{
  int n;

  n = buffer_read_ubyte(b);
  printf("Health Percentage %i\n",n);
  n = buffer_read_ubyte(b);
  printf("DEST Entry Number %i\n",n);
  n = buffer_read_ubyte(b);
  printf("Model Damage Stage %i\n",n);
  n = buffer_read_ubyte(b);
  printflags8(n);
  //      0000,0000
  printf("        ^Cap Damage\n");
  printf("       ^Disable Object\n");
  printf("      ^Destroy Object\n");
  n = buffer_read_ubyte(b);
  printf("Self Damage Per Second %i\n",n);
  n = buffer_read_ubyte(b);
  printf("%i (unused)\n",n);
  n = buffer_read_ubyte(b);
  printf("%i (unused)\n",n);
  ...
  printf("%i (unused)\n",n);
  n = buffer_read_int(b);
  printf("Explosion FormID %i\n",n);
  n = buffer_read_int(b);
  printf("Debris FormID %i\n",n);
  n = buffer_read_int(b);
  printf("Debris Count %i\n",n);
}

void parse_dmdl(void *b, int size)
{
  char *s2;

  s2 = buffer_read_nstring(b,size);
  printf("Replacement Model \"%s\"\n",s2);
  free(s2);
}

void parse_dmdt(void *b, int size)
{
  parse_modt(b,size); // XXXX we use parse_modt now
}

void parse_mods(void *b)
{
  int n;
  char *s2;
  int num;
...
#include "parsebuffer_arma.c"

#include "parsebuffer_armo.c"

#include "parsebuffer_aspc.c"

#include "parsebuffer_avif.c"

#include "parsebuffer_book.c"

#include "parsebuffer_bptd.c"

#include "parsebuffer_bsgn.c"

#include "parsebuffer_cams.c"
...
#include "parsebuffer_hdpt.c"

#include "parsebuffer_idle.c"

#include "parsebuffer_idlm.c"

#include "parsebuffer_imad.c"

#include "parsebuffer_imgs.c"

#include "parsebuffer_info.c"

#include "parsebuffer_ingr.c"

#include "parsebuffer_ipct.c"

#include "parsebuffer_ipds.c"

#include "parsebuffer_keym.c"

#include "parsebuffer_land.c"

#include "parsebuffer_lgtm.c"
...
#include "parsebuffer_lvln.c"

#include "parsebuffer_lvsp.c"

#include "parsebuffer_mesg.c"

#include "parsebuffer_mgef.c"

#include "parsebuffer_micn.c"

#include "parsebuffer_misc.c"

#include "parsebuffer_mstt.c"

#include "parsebuffer_musc.c"

#include "parsebuffer_navi.c"

#include "parsebuffer_navm.c"

#include "parsebuffer_note.c"

#include "parsebuffer_npc.c"

#include "parsebuffer_pack.c"

#include "parsebuffer_perk.c"

#include "parsebuffer_pgrd.c"

#include "parsebuffer_pgre.c"

#include "parsebuffer_proj.c"

#include "parsebuffer_pwat.c"

#include "parsebuffer_qust.c"

#include "parsebuffer_race.c"

#include "parsebuffer_rads.c"

#include "parsebuffer_refr.c"

#include "parsebuffer_regn.c"

#include "parsebuffer_rgdl.c"

#include "parsebuffer_road.c"

#include "parsebuffer_sbsp.c"

#include "parsebuffer_scol.c"
...
#include "parsebuffer_tact.c"

#include "parsebuffer_term.c"

#include "parsebuffer_tes4.c"

#include "parsebuffer_tree.c"

#include "parsebuffer_txst.c"

#include "parsebuffer_vtyp.c"
...
void parsebuffer(void *b)
{
  ...
  while (buffer_eof(b) != 1) {
	...
	break;
      ...
      printf("Stamp = %i\n",m);
      nn = buffer_read_int(b); // This was added to Fallout3.
      printf("%i\n",nn);

      // buffer_skip_bytes(b,n);
      buffer = buffer_getbuf(b);
      b2 = buffer_new(&(buffer[buffer_getloc(b)]),n-24); // 24 in Fallout3.
      global_locn++;
  ...
  printf("                                                ^TES4:Master\n");
  printf("                                         ^Has Tree LOD + GLOB:Constant + REGN:Border Region + REFR:Hidden From Local Map\n");
  printf("                                        ^REFR:Turn Off Fire\n");
  printf("                                      ^REFR:Inaccessible\n");
  printf("                                     ^On Local Map + REFR-LIGH:Casts Shadows + REFR:Motion Blur\n");
  printf("                                    ^Quest Item + LSCR:Displays in Main Menu + REFR:Persistent Reference + CELL:Persistent (CELL after WRLD)\n");
  printf("                                   ^REFR:Initially Disabled\n");
  printf("                                ^No Voice Filter\n");
  printf("                              ^Visible When Distant\n");
  printf("                            ^Random Anim Start + REFR:High Priority LOD\n");
  printf("                           ^Radio Station + ACTI:Dangerous + CELL:Off limits\n");
  printf("                          ^Compressed record data\n");
  printf("                         ^Can't Wait + ARMO,ACTI:Has Platform/Language Specific Textures\n");
  printf("                 ^Obstacle + REFR:No AI Acquire\n");
  printf("           ^---^^NavMesh Generation Import Option: 000 Collision Geometry 001 Filter 010 Bounding Box 100 Ground\n");
  printf("           ^---^^REFR: NavMesh Import Option: 000 Inherit From Base Object 011 Collision Geometry 001 Filter 010 Bound Box 100 Ground\n");
  printf("             ^Non-Pipboy\n");
  printf("             ^REFR:Reflected By Auto Water\n");
  printf("            ^Child Can Use\n");
  printf("            ^REFR:Refracted By Auto Water\n");
  printf("           ^Cont. Broadcast\n");
  printf("          ^Unknown\n");
  m = buffer_read_int(b);
  ...
  printf("flags 2 = ");
  printflags(flags2);
  n = buffer_read_int(b); // This was added to Fallout3.
  printf("flags 3 = ");
  printflags(n);
  printf("        = %i\n",n);

  buffer_setloc(b,mloc);
  buffer_skip_bytes(b,24); // 20 in Oblivion. 24 in Fallout3.
  ...
  if (flags1 & 0x00040000) {
	...
	r = parsebuffer_acti(b);
      } else if (strcmp(s,"ADDN") == 0) {
	r = parsebuffer_addn(b);
	...
	r = parsebuffer_appa(b);
      } else if (strcmp(s,"ARMA") == 0) {
	r = parsebuffer_arma(b);
      } else if (strcmp(s,"ARMO") == 0) {
	r = parsebuffer_armo(b);
      } else if (strcmp(s,"ASPC") == 0) {
	r = parsebuffer_aspc(b);
      } else if (strcmp(s,"AVIF") == 0) {
	r = parsebuffer_avif(b);
      } else if (strcmp(s,"BOOK") == 0) {
	r = parsebuffer_book(b);
      } else if (strcmp(s,"BPTD") == 0) {
	r = parsebuffer_bptd(b);
      } else if (strcmp(s,"BSGN") == 0) {
	r = parsebuffer_bsgn(b);
      } else if (strcmp(s,"CAMS") == 0) {
	r = parsebuffer_cams(b);
	...
	r = parsebuffer_flor(b);
      } else if (strcmp(s,"FLST") == 0) {
	r = parsebuffer_flst(b);
	...
	r = parsebuffer_hair(b);
      } else if (strcmp(s,"HDPT") == 0) {
	r = parsebuffer_hdpt(b);
      } else if (strcmp(s,"IDLE") == 0) {
	r = parsebuffer_idle(b);
      } else if (strcmp(s,"IDLM") == 0) {
	r = parsebuffer_idlm(b);
      } else if (strcmp(s,"IMAD") == 0) {
	r = parsebuffer_imad(b);
      } else if (strcmp(s,"IMGS") == 0) {
	r = parsebuffer_imgs(b);
      } else if (strcmp(s,"INFO") == 0) {
	r = parsebuffer_info(b);
      } else if (strcmp(s,"INGR") == 0) {
	r = parsebuffer_ingr(b);
      } else if (strcmp(s,"IPCT") == 0) {
	r = parsebuffer_ipct(b);
      } else if (strcmp(s,"IPDS") == 0) {
	r = parsebuffer_ipds(b);
      } else if (strcmp(s,"KEYM") == 0) {
	r = parsebuffer_keym(b);
      } else if (strcmp(s,"LAND") == 0) {
	r = parsebuffer_land(b);
      } else if (strcmp(s,"LGTM") == 0) {
	r = parsebuffer_lgtm(b);
	...
	r = parsebuffer_lvli(b);
      } else if (strcmp(s,"LVLN") == 0) {
	r = parsebuffer_lvln(b);
      } else if (strcmp(s,"LVSP") == 0) {
	r = parsebuffer_lvsp(b);
      } else if (strcmp(s,"MESG") == 0) {
	r = parsebuffer_mesg(b);
      } else if (strcmp(s,"MGEF") == 0) {
	r = parsebuffer_mgef(b);
      } else if (strcmp(s,"MICN") == 0) {
	r = parsebuffer_micn(b);
      } else if (strcmp(s,"MISC") == 0) {
	r = parsebuffer_misc(b);
      } else if (strcmp(s,"MSTT") == 0) {
	r = parsebuffer_mstt(b);
      } else if (strcmp(s,"MUSC") == 0) {
	r = parsebuffer_musc(b);
      } else if (strcmp(s,"NAVI") == 0) {
	r = parsebuffer_navi(b);
      } else if (strcmp(s,"NAVM") == 0) {
	r = parsebuffer_navm(b);
      } else if (strcmp(s,"NOTE") == 0) {
	r = parsebuffer_note(b);
      } else if (strcmp(s,"NPC_") == 0) {
	r = parsebuffer_npc(b);
      } else if (strcmp(s,"PACK") == 0) {
	r = parsebuffer_pack(b);
      } else if (strcmp(s,"PERK") == 0) {
	r = parsebuffer_perk(b);
      } else if (strcmp(s,"PGRD") == 0) {
	r = parsebuffer_pgrd(b);
      } else if (strcmp(s,"PGRE") == 0) {
	r = parsebuffer_pgre(b);
      } else if (strcmp(s,"PROJ") == 0) {
	r = parsebuffer_proj(b);
      } else if (strcmp(s,"PWAT") == 0) {
	r = parsebuffer_pwat(b);
      } else if (strcmp(s,"QUST") == 0) {
	r = parsebuffer_qust(b);
      } else if (strcmp(s,"RACE") == 0) {
	r = parsebuffer_race(b);
      } else if (strcmp(s,"RADS") == 0) {
	r = parsebuffer_rads(b);
      } else if (strcmp(s,"REFR") == 0) {
	r = parsebuffer_refr(b);
      } else if (strcmp(s,"REGN") == 0) {
	r = parsebuffer_regn(b);
      } else if (strcmp(s,"RGDL") == 0) {
	r = parsebuffer_rgdl(b);
      } else if (strcmp(s,"ROAD") == 0) {
	r = parsebuffer_road(b);
      } else if (strcmp(s,"SBSP") == 0) {
	r = parsebuffer_sbsp(b);
      } else if (strcmp(s,"SCOL") == 0) {
	r = parsebuffer_scol(b);
	...
	r = parsebuffer_stat(b);
      } else if (strcmp(s,"TACT") == 0) {
	r = parsebuffer_tact(b);
      } else if (strcmp(s,"TERM") == 0) {
	r = parsebuffer_term(b);
      } else if (strcmp(s,"TES4") == 0) {
	r = parsebuffer_tes4(b);
      } else if (strcmp(s,"TREE") == 0) {
	r = parsebuffer_tree(b);
      } else if (strcmp(s,"TXST") == 0) {
	r = parsebuffer_txst(b);
      } else if (strcmp(s,"VTYP") == 0) {
	r = parsebuffer_vtyp(b);
	...
	printf("Record %s unimplemented. Skipping the record.\n",s);
	exit(-1); // exit.
	// NOT NEED buffer_skip_bytes(b,recordsize);
  ...
  b = btmp;
  buffer_setloc(b,mloc);
  buffer_skip_bytes(b,24); // 24 in Fallout3.
