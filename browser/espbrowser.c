...
// Library version: parsing and printing separated.
void create_global_records(void *b)
{
  ...
  while (buffer_eof(b) != 1) {
    ...
    buffer_setloc(b,loc);
    // XXXX new
    r = buffer_parse_block(b,"recgroup");
    data = block_get("recgroup","type");
    // XXXX CONTINUE check type, interpret
    // CONTINUE
    // CONTINUE
    // CONTINUE
...
// 20121129 working version
void create_global_records_old(void *b)
{
  int loc;
  char *type;
  unsigned int formid;
