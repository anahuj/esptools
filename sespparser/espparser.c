...
// Copyright 2007-2012 Juhana Sadeharju
// GNU GPL
...
void parsebuffer(void *b)
{
  ...
  while (buffer_eof(b) != 1) {
    loc = buffer_getloc(b);
    fprintf(stderr,"loc at begin of record %i\n",loc);
	...
	break;
      ...
      b2 = buffer_new(&(buffer[buffer_getloc(b)]),n-24); // XXXX 24
      global_locn++;
  ...
  buffer_skip_bytes(b,24); // XXXX 24
