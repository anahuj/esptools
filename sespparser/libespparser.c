...
void esp_read_name_in(void *pp, char *s)
{
  void *b;
  ty_esp *p;
...
void esp_skip_record(void *pp)
{
  ...
  buffer_skip_bytes(b,n+20-8); // 8 = RecordID + recordsize, 20 = headersize
  // loc = buffer_getloc(b);
  // printf("loc = %i\n",loc);
}

void esp_skip_subrecord(void *pp)
{
  void *b;
  int n;
  ty_esp *p;
...
void esp_print_grouptype(void *pp)
{
  void *b;
  int n;
  int loc;
  ...
  switch (grouptype) {
  case 0:
    printf("  %s %s\n",label_s,p->grouptypes[grouptype]);
    break;
  case 1:
    printf("  %i %s\n",label_int,p->grouptypes[grouptype]);
    break;
  case 2:
    printf("  %i %s\n",label_int,p->grouptypes[grouptype]);
    break;
  case 3:
    ...
    break;
  case 4:
    printf("  %i %i %s\n",label_x,label_y,p->grouptypes[grouptype]);
    break;
  case 5:
    printf("  %i %i %s\n",label_x,label_y,p->grouptypes[grouptype]);
    break;
...
void esp_enter_group(void *pp)
{
  ...
  buffer_skip_bytes(b,20-8); // Header size = 20.

  b2 = buffer_new(buffer_getbuf_atloc(b),groupsize-20); // Includes header size.
...
void esp_enter_record(void *pp)
{
  ...
  buffer_skip_bytes(b,20-4-8); // Header size = 20. 4 = RecID.
...
int esp_get_recordsize(void *pp)
{
  void *b;
  int n;
  int loc;
...
int esp_get_subrecordsize(void *pp)
{
  void *b;
  int n;
  int loc;
...
#if 0
// Working version pre 20100326, prior reading header data to data structures.
...
#if 1
// Working version post 20100326. Reads the header data to data structures.

// Record header has common flags, and flags related to the record type.
...
// Subrecords are read when needed. They are not kept in the memory.
typedef struct {
} ty_subrecord;
